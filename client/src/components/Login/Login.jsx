...
function Login() {
    ...
    const handleSubmit = async (e) => {
        ...
        const data = await axios.post("http://localhost:3456/api/session/login", data_to_send)
  ...
  return (
    <div className='flex flex-col w-full items-center '>
        <h1 className='text-2xl font-bold text-center my-5'>Inicia sesión</h1>
        <form className=' flex flex-col ' onSubmit={handleSubmit}>
            ...
            {password === "" || username === "" ? <p className='my-5 bg-zinc-800 text-zinc-700 w-48 mx-auto py-2 rounded-md text-center font-bold'>Inicia sesión</p> : <button className='my-5 bg-red-500 w-48 mx-auto py-2 rounded-md font-bold'>Inicia sesión</button>}
        </form>
    </div>
  )
}
