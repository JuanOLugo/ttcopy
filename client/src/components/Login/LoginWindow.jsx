...
function LoginWindow({setOpen}) {
  ...
  return (
    <div className='bg-black bg-opacity-30 text-white  w-screen h-screen z-10 absolute top-0 flex items-center'> 
        <div className='bg-zinc-900 w-2/6 h-5/6 mx-auto rounded-lg flex flex-col justify-between'>
        <button className='absolute text-xl font-bold m-3 text-zinc-700' onClick={() => {
            setOpen(false)
        }}>X</button>
        {!sOl ? <Login/> : <Singup/>}
