...
function EditVideo({ file }) {
  ...
  const uploadFile = async (dataToSend) => {
    ...
    const data = await axios.post(
      "http://localhost:3456/api/upload/video",
      formData,
      {
        headers: {
          ...
          <div className="bg-zinc-800 px-4 py-3 rounded-lg">
            ...
            <div>
              <h1 className="text-white text-xl font-normal">
                Tamaño: {size} {Number(size) > 1024000000000 ? "GB" : "MB"}
              </h1>
            </div>
          </div>
          <div className="bg-zinc-800 px-4 py-3 rounded-lg flex justify-evenly items-center my-10">
            <div className="bg-zinc-700 w-6/12 rounded-md p-2">
              <textarea
                ...
                className="resize-none outline-none w-96 h-28 bg-zinc-700 rounded-md p-3 placeholder:text-white text-white font-semibold"
                placeholder="Ingresa una descripción"
              ></textarea>
              <button
                className="text-zinc-400 cursor-pointer font-bold"
                onClick={() => {
